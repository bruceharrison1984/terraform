...
func (s MultiSource) AvailableVersions(provider addrs.Provider) (VersionList, error) {
	...
	if len(vs) == 0 {
		if registryError {
			...
			return nil, ErrProviderNotKnown{provider, s.sourcesForDisplay(provider)}
		}
	}
...
// sourcesForDisplay returns a user-friendly display string of sources
// configured for a given provider, intended to be used as the Description of an
// ErrProviderNotKnown
func (s MultiSource) sourcesForDisplay(provider addrs.Provider) string {
	sources := s.sourcesForProvider(provider)
	ret := make([]string, len(sources))
	for i, source := range sources {
		ret[i] = fmt.Sprintf("- %s", source)
	}
	return strings.Join(ret, "\n")
}
